...
<div id="all">
    ...
    <div id="show-message">
        ...
        <form action="<%=request.getContextPath()%>/admin/doUpdateSinger" method="post" enctype="multipart/form-data" charset=utf-8" >
            <table id = "add-singer-mes">
                <tr><th class="show-name">资料</th>
                    <th class="show-username"></th></tr>
                <tr><td >歌手名</td>
                    <td><input type="text" name="singerName" required="required" value="${singer.singerName}"/></td></tr>
                <tr><td >国籍</td>
                    <td>
                        <p>${singer.singerCountry}</p>
                        <%--<select id="singer-country" name="singerCountry" required="required" >--%>
                        <%--<option value="${singer.singerCountry}">${singer.singerCountry}</option>--%>
                ...
                <tr><td>男/女/组合</td>
                    <td>
                        <%--<select id="singer-sex" name="singerSex" required="required">--%>
                        <%--<input type="text" value="${singer.singerSex}"/>--%>
                        <p>${singer.singerSex}</p>
                ...
                <tr><td >类别</td>
                    <td><select id="singer-style" name="singerStyle" required="required">
                        <option value="${singer.singerStyle}">${singer.singerStyle}</option>
                ...
                <tr><td>描述信息</td>
                    <td>
                        <%--<p>${singer.singerSummary}</p>--%>
                        <input type="text" name="singerSummary" id="singer-message" value="${singer.singerSummary}"/>
                    </td></tr>
                <tr><td>照片</td>
                    <td>
                        <img src="/files/${singer.singerPicture}" alt="" class="show-shoppic" id="showpic"/>
                        <input type="file" class="" name="file1" id="uploadfile1" onchange="changepic3(this)"  accept="image/png, image/jpeg, image/jpg" />
                    </td></tr>
                <tr><td></td><td><input type="hidden" value="${singer.id}" name="id"/></td></tr>
                <tr><td>操作</td><td><input type="submit" value="保存修改"/></td></tr>
            </table>
        </form>
    </div>
</div>
