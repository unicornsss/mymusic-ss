...
window.onload = function() {
    new Selected().init();
    //var valuesss = document.getElementById('PageContext').value;
    ////alert(valuesss);
...
Selected.prototype = {
    ...
    init: function() {
        ...
        randomSong = currentSong.children[0].getAttribute('data-songname');

        randomSongmp3 = currentSong.children[0].getAttribute('data-mp3file');
        randomLrc = currentSong.children[0].getAttribute('data-name');

        //alert(randomSong);
        //alert(randomLrc);
        if(randomLrc == null){
            randomLrc = "default_lrc.lrc"
        }
        //alert(randomSongmp3);
        ...
        this.playlist.addEventListener('click', function(e) {
            ...
            var songmp3 = e.target.getAttribute('data-mp3file');
            var songlrc = e.target.getAttribute('data-name');
            if(songlrc == null){
                songlrc = "default_lrc.lrc"
            }

            window.location.hash = songmp3;
            that.play(songmp3,songlrc);
        }, false);
        ...
        this.play(randomSongmp3,randomLrc);
    },
    initialList: function(ctx) {
        ...
        xhttp.open('GET', '/other/content.json', false);
        xhttp.onreadystatechange = function() {
            if (xhttp.status == 200 && xhttp.readyState == 4) {
                ...
                data.forEach(function(v, i, a) {
                    ...
                    a.dataset.songname = v.song_name;
                    //default_lrc.lrc

                    a.dataset.name = v.lrc_name;
                    if(v.lrc_name == null){
                        a.dataset.name = "default_lrc.lrc";
                    }
                    a.dataset.mp3file = v.mp3file;
                    a.dataset.songId = v.songId;
    ...
    play: function(songName,songlrc) {
        ...
        this.audio.src = '/music/' + songName ;
        this.audio.lrc= '/lrc/' + songlrc ;
    ...
    playNext: function(that) {
        ...
        var songmp3 = nextItem.getAttribute('data-mp3file');
        var songlrc = nextItem.getAttribute('data-name');
