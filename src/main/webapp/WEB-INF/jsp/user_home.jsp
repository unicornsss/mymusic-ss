...
<div class="show-img">
    <div >
        <img src="/files/${userFind.userPic}" alt="" id="show-img-self"/>
    </div>
    <h1>${userFind.username}</h1>
    <ul class="show-fan">
        <c:if test="${noticeFlag == true}">
            <button id="notice-user">已关注</button>
        </c:if>
        <c:if test="${noticeFlag == false}">
            <button id="notice-user">关注</button>
        </c:if>
        ...
        <li>
            <div class="every-song">
                <div class="every-header">
                    <a href="/music/showEverySong?songId=${song.id}" class="song-name"><span>${song.songName}</span></a>
                    <div class="btn-play">
                ...
                <span class="every-singer"><a href="/music/showEverySinger?singerId=${song.singer.id}">${song.singer.singerName}</a></span>
                <span class="every-spe"><a href="/music/showEverySpe?speId=${song.special.id}">${song.special.specialName}</a>
