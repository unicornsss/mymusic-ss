...
<div id="all">
    ...
    <div id="show-message">
        ...
        <div id="showipe">
            <c:forEach items="${specialList}" var="specialList">
                <div class="show-every-singer">
                    <a href="/admin/showSong?sId=${specialList.id}" >
                            <%--<p2>/files/${singerList.singerPicture}</p2>--%>
                        <img src="/files/${specialList.specialPicture}" alt="" class="show-pic"/>
                        <p>${specialList.specialName}---${specialList.singer.singerName}</p>
                            <%--<p><a href="/admin/updateSinger/?singerId=${singerList.id}">修改</a></p>--%>
                    </a>
                    <a href="/admin/deleteSpecial?speId=${specialList.id}" ><span>删除</span></a>
                </div>
            </c:forEach>
        </div>
    </div>
